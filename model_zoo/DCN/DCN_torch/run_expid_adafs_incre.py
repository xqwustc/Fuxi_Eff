...
if __name__ == '__main__':
    ...
    # torch.save(model.state_dict(), f"{params['model']}_warmup4ada.pth")
    # logging.info('****** Warmup end and save model in {} ******'.format(f"{params['model']}_warmup4ada.pth"))
    ...
    logloss = []
    #for i in []:
    for i in range(9, len(feature_map.features)):
        model_class = getattr(model_zoo, params['model'])
        print('params[model]', params['model'])
        model = model_class(feature_map, select_num= i+1, **params)
        model.load_state_dict(torch.load(f"{params['model']}_warmup4ada.pth"), strict=False)
        select_nums.append(i+1)
        model.count_parameters()  # print number of parameters used in model
        logging.info('Used Feature Number:{} '.format(i+1))
        train_gen, valid_gen = H5DataLoader(feature_map, stage='train', **params).make_iterator()
        if params['mode'] == 2:
            # hard select, select number is needed
            pparam = copy.deepcopy(params)
            pparam['select_num'] = i + 1
            model.fit_for_adafs(train_gen,validation_data = valid_gen,**pparam)
        else:
