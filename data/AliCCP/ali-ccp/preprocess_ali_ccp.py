...
if __name__ == "__main__":
    ...
    # 第一步：划分训练集和临时数据集（包含验证集和测试集）
    train_data, temp_data = train_test_split(train_data, test_size=0.2, random_state=2024)
    # 第二步：将临时数据集划分为验证集和测试集（各占50%）
    val_data, test_data = train_test_split(temp_data, test_size=0.5, random_state=2024)
